...
void comic_death_animation();
void comic_dies(uint8_t death_by_hp);
uint8_t check_horizontal_enemy_map_collision(uint8_t x, uint8_t y);
uint8_t check_vertical_enemy_map_collision(uint8_t x, uint8_t y);
...
void play_sound(uint16_t sound[][2], size_t sound_size)
{
  if(sound_enabled)
      ...
      for (size_t note_counter = sound[note][1] * SAMPLE_RATE / 18; i < buffer_length; i++, note_counter--)
	{
	  ...
	  audio_buffer[i] = ((size_t)(i * frequency * 2.0 / SAMPLE_RATE) % 2)*2048;
	  if (note_counter == 0)
	    {
	      note++;
...
void init_sound()
{
  ...
  SDL_setenv("SDL_AUDIODRIVER", "directsound", 1); // use directsound on windoof
#endif
...
void enemy_behavior_bounce(enemy * foe)
{
  // Should this enemy skip this tick?
  switch(foe->restraint)
    {
    case ENEMY_RESTRAINT_MOVE_THIS_TICK:
      
      // This enemy is a slow enemy in ENEMY_RESTRAINT_MOVE_THIS_TICK state.
      // Transition to ENEMY_RESTRAINT_SKIP_THIS_TICK.
      foe->restraint = ENEMY_RESTRAINT_SKIP_THIS_TICK;
    case ENEMY_RESTRAINT_MOVE_EVERY_TICK:
      //Horizontal
      if(foe->x_vel > 0) // moving left or right?
	{ //moving right
	  foe->facing = ENEMY_FACING_RIGHT;
	  // check the solidity of the tile at (enemy.x + 2, enemy.y) (and the one below it, if enemy.y is odd)
	  (foe->x)++; // provisionally set enemy.x = enemy.x + 1
	  if(check_horizontal_enemy_map_collision(foe->x + 1, foe->y))
	    { // hit a solid map tile, undo the enemy.x update
	      // We hit a solid tile or the right edge of the playfield, while moving
	      // right.
	      (foe->x)--;
	      foe->x_vel = -1;// bounce back to the left
	    }
	  else
	    { // check playfield right
	      int8_t screen_x = foe->x - camera_x;
	      if(screen_x >= PLAYFIELD_WIDTH - 2)
		{
		  foe->x_vel = -1; // hit the right edge; bounce back
		}
	    }
	}
      else
	{ // moving left
	  foe->facing = ENEMY_FACING_LEFT;
	  // hit the left side of the map?
	  if(foe->x == 0)
	    {
	      foe->x_vel = 1;
	    }
	    ...
	    {
	      (foe->x)--; // provisionally set enemy.x = enemy.x - 1
	      if(check_horizontal_enemy_map_collision(foe->x, foe->y)) 	// check the solidity of the tile at (enemy.x - 1, enemy.y) (and the one below it, if enemy.y is odd)
		{ // hit a solid map tile, undo the enemy.x update
		  (foe->x)++;
		  foe->x_vel = 1; // bounce back to the right
		}
	      else
		{ // no collision, keep the enemy.x update
		  if(foe->x <= camera_x) // left side of the map
		    foe->x_vel = 1; // bounce back to the right
		}
	    }
	}
      // vertical
      if(foe->y_vel > 0) // moving down or up?
	{ // moving down
	  if(foe->y >= PLAYFIELD_HEIGHT - 2) // hit the bottom of the playfield?
	    { // bounce up
	      foe->y_vel = -1;
	    }
	  else
	    {
	      (foe->y)++;
	      if(check_vertical_enemy_map_collision(foe->x, foe->y + 1)) // check the solidity of the tile at (enemy.x, enemy.y + 2) (and the one to its right, if enemy.x is odd)
		{ // hit a solid map tile, undo the enemy.y update
		  (foe->y)--;
		  foe->y_vel = -1;
		}
	      else
		{ // no collision, keep the enemy.y update
	...
	{ // moving up
	  ...
	  if(foe->y == 0)
	    ...
	    {
	      (foe->y)--; // provisionally set enemy.x = enemy.x - 1
	      if(check_vertical_enemy_map_collision(foe->x, foe->y)) 	// check the solidity of the tile at (enemy.x - 1, enemy.y) (and the one below it, if enemy.y is odd)
		{ // hit a solid map tile, undo the enemy.y update
		  (foe->y)++;
		  foe->y_vel = 1; // bounce back down
		}
	      else
		{ // no collision, keep the enemy.y update
		  if(foe->y == 0) // top of the playfield?
		    foe->y_vel = 1; // bounce back down
		}
	    }
	}
      break;
    case ENEMY_RESTRAINT_SKIP_THIS_TICK:
      // This is a slow enemy that just skipped a tick; move the next tick.
      foe->restraint = ENEMY_RESTRAINT_MOVE_THIS_TICK;
    default: break;
    }
  // done
  return;
...
void enemy_behavior_roll(enemy * foe)
{
  enemy_behavior_bounce(foe);
}

void enemy_behavior_seek(enemy * foe)
{
  enemy_behavior_bounce(foe);
}

void enemy_behavior_shy(enemy * foe)
{
  enemy_behavior_bounce(foe);
}
...
void maybe_spawn_enemy(enemy * foe)
{
  foe->spawn_timer_and_animation = 0;
  // has an enemy already been spawned this tick?
  if(enemy_spawned_this_tick == 0) // if so, don't spawn any more this tick
    {
      ...
      enemy_respawn_position_cycle += 2;
      if(enemy_respawn_position_cycle >= PLAYFIELD_WIDTH + 7) // hit the top of the cycle?
	enemy_respawn_position_cycle = PLAYFIELD_WIDTH;
	...
	{ // try spawn left
	  ...
	  // playfield.
	  spawn_x = camera_x + (PLAYFIELD_WIDTH - 2) - enemy_respawn_position_cycle;
	}
      else
	{
	  spawn_x = camera_x + enemy_respawn_position_cycle;
	  ...
	  // ; jo .done
	  // TODO: finish
	  if(camera_x <= 127 && camera_x + enemy_respawn_position_cycle > 128)
	    {
	      return;
	    }
	}
      ...
      // consecutive.
      uint8_t spawn_y_start = comic_y & 0xfe; // clamp to an even tile boundary
      spawn_y_start += 4; // start looking for a vertical position at the level of Comic's feet, rounded towards the top of the playfield
      // First look for a solid tile.
      uint8_t look_for_solid = 1;
      for(int8_t spawn_y = spawn_y_start; spawn_y >= 0; spawn_y-=2)
	{
	  if(look_for_solid)
	    {
	      look_for_solid = !is_solid(spawn_x, spawn_y); // found a solid tile, now look for a non solid tile
	    }
	  else // look for a non-solid tile above
	    {
	      if(!is_solid(spawn_x, spawn_y))
		{ // not solid, this is the spot
		  // .spawn
		  enemy_spawned_this_tick = 1;
		  foe->x = spawn_x; // initialize enemy.y and enemy.x
		  foe->y = spawn_y;
		  foe->state = ENEMY_STATE_SPAWNED;
		  if(foe->behavior == ENEMY_BEHAVIOR_BOUNCE || foe->behavior == ENEMY_BEHAVIOR_SHY)
		    { // ENEMY_BEHAVIOR_BOUNCE and ENEMY_BEHAVIOR_SHY start out moving up and
		      // left.
		      foe->x_vel = -1;
		      foe->y_vel = -1;
		    }
		  else if(foe->behavior != ENEMY_BEHAVIOR_UNUSED)
		    ...
		    { // We tried to spawn an unused enemy slot (enemy.behavior ==
		      ...
		      foe->state = ENEMY_STATE_DESPAWNED;
		      foe->spawn_timer_and_animation = 100; // try this slot again after 100 ticks
		    }
		  break;
		}
	    }
...
{
  enemy_spawned_this_tick = 0;
  for(int i = 0; i < MAX_NUM_ENEMIES; i++) // for each enemy slot
    {
      enemy * foe = &enemies[i];
      // is this enemy spawned, despawned, or dying?
      if(foe->state == ENEMY_STATE_SPAWNED)
	{ // spawned // This is a spawned enemy. Advance the animation counter.
	  foe->spawn_timer_and_animation++;
	  if(foe->spawn_timer_and_animation >= foe->num_animation_frames) // animation counter hit the number of animation frames?
	    foe->spawn_timer_and_animation = 0; //wrap back to animation frame 0
	  // Branch based on enemy.behavior.
	  // jump back to .check_despawn when finished.
	  switch(foe->behavior)
	    {
	    ...
	    case ENEMY_BEHAVIOR_LEAP:
	      enemy_behavior_leap(foe);
	      break;
	    case ENEMY_BEHAVIOR_ROLL:
	      enemy_behavior_roll(foe);
	      break;
	    case ENEMY_BEHAVIOR_SEEK:
	      enemy_behavior_seek(foe);
	      break;
	    case ENEMY_BEHAVIOR_SHY:
	      enemy_behavior_shy(foe);
	      break;
	    default: continue; // skip this enemy if its behavior is unknown
	    }
	  // check_despawn
	  uint8_t distance = abs(foe->x - comic_x);
	  if(distance >= ENEMY_DESPAWN_RADIUS)
	    {
	      goto despawn;
	    }
	  ...
	  // direction.
	  if(abs(foe->x - comic_x) < 2
	     && !(foe->y - comic_y < 0)
	     && !(foe->y - comic_y >= 4))
	    { // enemy collides with comic
	      foe->state = ENEMY_STATE_RED_SPARK; // start the enemy's red spark animation.
	      // does this hit kill comic?
	      if(comic_hp != 0 // don't re-enter Comic's death animation if it is already in progress 
		...
		{ // kill comic
		  ...
		  comic_death_animation(); // calls recursively back into handle_enemies
		  inhibit_death_by_enemy_collision = 0;
		  comic_dies(1);
		  return;
		}
	  ...
	  int16_t camera_relative_x = foe->x - camera_x;
	  if(camera_relative_x >= 0
	     && camera_relative_x <= PLAYFIELD_WIDTH - 2)
	    {
	      blit(16, 16, // width and height
		   0, 0, // source x and y
		   8+(camera_relative_x)*8, 8+foe->y*8, // target x and y 
		   16, foe->animation_frames_ptr[foe->spawn_timer_and_animation + foe->facing], // source buffer
		   SCREEN_WIDTH, screen_buffer); // target buffer
	    }
	  continue;
	}
      else if(foe->state == ENEMY_STATE_DESPAWNED)
	{ // despawned
	  // advance the spawn timer
	  foe->spawn_timer_and_animation--;
	  if(foe->spawn_timer_and_animation < 0)
	    { // Spawn counter is finished; try to spawn the enemy in this slot.
	      maybe_spawn_enemy(foe);
	    }
	  continue;
	}
    dying:
      ...
      if(!(foe->x - camera_x < 0 || foe->x - camera_x > PLAYFIELD_WIDTH - 2))
	{
	  if(foe->state >= 2 && foe->state < 7)
	    { // in white spark animation
	      if(foe->state <= 2+2)
		{ // blit enemy under spark
		  blit(16, 16, // width and height
		       0, 0, // source x and y
		       8+(foe->x - camera_x)*8, 8+foe->y*8, // target x and y 
		       16, ANIMATION_TABLE_SPARKS[foe->state], // source buffer
		       SCREEN_WIDTH, screen_buffer); // target buffer
		}
	      // blit spark
	      blit(16, 16, // width and height
	  ...
	  else if (foe->state >= 8 && foe->state < 12)
	    { // in red spark animation
	      if(foe->state <= 8+2)
		{ // blit enemy under spark
		  blit(16, 16, // width and height
		       0, 0, // source x and y
    ...
    despawn:
      ...
      foe->spawn_timer_and_animation = enemy_respawn_counter_cycle;
      enemy_respawn_counter_cycle+=20;
      if(enemy_respawn_counter_cycle <= 120)
	enemy_respawn_counter_cycle = 20;
    }
}
...
void handle_fireballs()
{
  if(comic_firepower != 0)
    {
      for(int i = 0; i < comic_firepower; i++)
	{
	  // Is this slot active?
      ...
      // enemy.
      for(int i = 0; i < MAX_NUM_FIREBALLS; i++) //for each fireball slot (not limited to comic_firepower here for some reason)
	{
	  fireball *ball = &fireballs[i];
	  if(!(ball->x == FIREBALL_DEAD || ball->y == FIREBALL_DEAD))
	    {
	      for(int j = 0; j < MAX_NUM_ENEMIES; j++) // for each enemy slot
		{
		  enemy *foe = &enemies[j];
		  if(foe->state == ENEMY_STATE_SPAWNED)
		    {
		      
		      int8_t d_y = ball->y - foe->y; // vertical distance must be 0 or 1
		      int8_t d_x = ball->x - foe->x; // Horizontal distance must be -1, 0, or +1.
		      if((d_y == 0 || d_y == 1)
			 && (d_x == -1 || d_x == 0 || d_x == 1))
			{ // Found a fireball–enemy collision.
			  foe->state = ENEMY_STATE_WHITE_SPARK; // start the enemy's white spark animation
...
void load_new_stage()
 {
   ...
   for(size_t i = 0; i < MAX_NUM_ENEMIES; i++)
     {
       enemy *foe = &enemies[i];
       if(current_stage.enemies[i].behavior == ENEMY_BEHAVIOR_UNUSED)
	 { // invalid shp index
	   foe->animation_frames_ptr = 0;
	   foe->behavior = ENEMY_BEHAVIOR_UNUSED;
	   foe->facing = ENEMY_FACING_LEFT;
	   foe->num_animation_frames = 0;
	   foe->restraint = 0xFF;
	   foe->spawn_timer_and_animation = 0;
	   foe->state = ENEMY_STATE_DESPAWNED;
	   foe->x = 0;
       ...
       else //valid_shp_index
	 {
	   foe->behavior = current_stage.enemies[i].behavior;
	   foe->num_animation_frames = enemy_num_animation_frames[i];
	   foe->animation_frames_ptr = enemy_animation_frames[current_stage.enemies[i].shp_index];
	   foe->state = ENEMY_STATE_DESPAWNED;
	   foe->facing = ENEMY_FACING_LEFT;
	   foe->spawn_timer_and_animation = 20; // all enemies scheduled to spawn 20 ticks after entering a new stage
	   foe->restraint = ENEMY_RESTRAINT_MOVE_THIS_TICK; // initially assume it's a slow enemy
	   if(ENEMY_BEHAVIOR_FAST & foe->behavior) // is the ENEMY_BEHAVIOR_FAST bit set?
	     {
	       foe->behavior -= ENEMY_BEHAVIOR_FAST; // unset the ENEMY_BEHAVIOR_FAST bit
	       foe->restraint = ENEMY_RESTRAINT_MOVE_EVERY_TICK; // fast enemies move every tick
	     }
	   foe->x = 0;
	   foe->x_vel = 0;
	   foe->y = 0;
